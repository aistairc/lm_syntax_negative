...
class SplitCrossEntropyLoss(nn.Module):
    ...
    def __init__(self, hidden_size, splits, verbose=False, ignore_index=-100):
    ...
    def logprob(self, weight, bias, hiddens, splits=None, softmaxed_head_res=None, verbose=False):
        ...
        if softmaxed_head_res is None:
            ...
            softmaxed_head_res = torch.nn.functional.log_softmax(head_res) # (words,ntokens)
    ...
    def split_on_targets(self, hiddens, targets):
        ...
        return split_targets, split_hiddens, tmp_masks

    def forward(self, weight, bias, hiddens, targets, verbose=False, reduce=True):
        ...
        if self.ignore_index >= 0:
            ...
            targets = targets.masked_select(ignore_mask)
            hiddens = hiddens[ignore_mask.nonzero().squeeze(1)]

        split_targets, split_hiddens, tmp_masks = self.split_on_targets(hiddens, targets)
        ...
        for idx in range(self.nsplits):
            ...
            # If the target is in one of the splits, the probability is the p(tombstone) * p(word within tombstone)
                ...
                entropy = -(head_entropy + tail_entropy).unsqueeze(1)
            ###
            ...
            if reduce:
                ...
                if self.nsplits > 1:
                    unmasked_entropy = entropy.new_zeros([targets.size(0), 1])
                    unmasked_entropy[tmp_masks[idx]] = entropy
                else:
                    unmasked_entropy = entropy
                total_entropy = unmasked_entropy.float() if total_entropy is None else total_entropy + unmasked_entropy.float()

        if reduce:
            # return (total_entropy.sum() / len(targets)).type_as(weight)
            return (total_loss / len(targets)).type_as(weight)
        else:
            return total_entropy

    def sample(self, weight, bias, hidden, temperature, ignore=None):
        # We perform hierarchical sampling, by first sample on the head vocab and tombstons.
        # If the sample hits on tombstons then we further sample from the multinomial of that
        # tombstone.
        #
        # Assume that hidden is already squeezed the batch dimension.
        #
        # `ignore` is a list of indexes, in which probability gets zero.
        ...
        head_res = torch.nn.functional.linear(hidden, head_weight, bias=head_bias)
        head_res = head_res.div(temperature).exp()
        idx = torch.multinomial(head_res, 1)[0]
        if idx >= end:
            split_idx = idx - end + 1
            if (split_idx > self.nsplits):
                print('split_idx {} nsplits {}'.format(split_idx, self.nsplits))
            assert split_idx <= self.nsplits

            start, end = self.splits[split_idx], self.splits[split_idx + 1]
            tail_weight = weight[start:end]
            tail_bias = bias[start:end]

            tail_res = torch.nn.functional.linear(hidden, tail_weight, bias=tail_bias)
            tail_res = tail_res.div(temperature).exp()
            idx = torch.multinomial(tail_res, 1)[0]
            idx = start + idx
        return idx

    def log_dist(self, weight, bias, hiddens):
        """Return distribution for the next word to calculate entropy of the current context."""

        # hidden = hidden.unsqueeze(0)
        return self.logprob(weight, bias, hiddens)
        # return self.logprob(weight, bias, hiddens)[0]

    def entropy(self, weight, bias, hiddens):
        """hiddens: [batch*length, unit]"""
        assert(hiddens.dim() == 2)
        log_dist = self.log_dist(weight, bias, hiddens).detach()
        dist = log_dist.exp().detach() # this should sum to 1.0?
        return -(log_dist * dist).sum(1)
